...
const COLS = `id, project_id, title, description, note, type, department, assignee_email, created_by,
  status, priority, start_date, end_date, due_date, created_at, updated_at`;
...
export async function PATCH(req, { params }) {
  ...
  const { rows } = await query(
    `update ext.task set title=$2, description=$3, note=$4, type=$5, status=$6, priority=$7,
       start_date=$8, end_date=$9, assignee_email=$10, project_id=$11, department=$12, updated_at=now()
     where id=$1 returning ${COLS}`,
    [id, title, description, note, type, status, priority, start_date, end_date, assignee, project_id, department],
  );
