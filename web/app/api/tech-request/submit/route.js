...
export async function POST(request) {
  ...
  const ag = (await query(
    'select agreement_type, salesman_email, uploaded_by from ops.legal_agreement where id = $1', [agreementId]
  )).rows[0];
  if (!ag || !canSee(user, ag)) return NextResponse.json({ error: 'agreement not found' }, { status: 404 });
